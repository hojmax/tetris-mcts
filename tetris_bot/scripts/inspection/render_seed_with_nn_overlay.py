...
class ScriptArgs:
    ...
    visit_sampling_epsilon: float | None = None  # Override config; 0 = greedy argmax
    top_moves: int = 3  # Number of top-prior moves to overlay in the *_top3_moves gif
...
def resolve_move_overlay_output_path(output_path: Path, top_moves: int) -> Path:
    return output_path.with_name(
        f"{output_path.stem}_top{top_moves}_moves{output_path.suffix}"
    )
...
def main(args: ScriptArgs) -> None:
    ...
    if args.visit_sampling_epsilon is not None:
        config.visit_sampling_epsilon = float(args.visit_sampling_epsilon)
    ...
    move_overlay_output_path = resolve_move_overlay_output_path(output_path, args.top_moves)
    plain_frames = []
    move_overlay_frames = []
    frame_values: list[float] = []
    total_attack = 0
    final_total_attack = int(playback.total_attack)
    final_total_placements = (
        int(playback.steps[-1].placement_count) + 1 if playback.steps else 0
    )
